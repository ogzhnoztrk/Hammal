...
namespace HammalWeb.Areas.Customer.Controllers
{
    ...
    public class ServiceController : Controller
    {
        ...
        public IActionResult OrderCard(int systemUserId)
        {
            ...
            order.SystemUserId = systemUserId;
            order.OdemeDurum = SD.Odeme_Yapilmadi;
            order.SiparisDurum = SD.Siparis_Olusturulmadi;
            order.OrderTotal = (double)_unitOfWork.SystemUser.GetFirstOrDefault(i => i.Id == systemUserId).Price;


            


            return RedirectToAction("OrderCard", "Order", order);
        }
